...
:: 3. Pilih interpreter Python yang benar-benar punya dependensi SELA (fastapi + uvicorn)
::    Catatan: `python` di PATH bisa menunjuk versi lain tanpa paket, sehingga server gagal jalan.
set "PYEXE=python"
py -3.12 -c "import fastapi, uvicorn" >nul 2>&1
if %ERRORLEVEL% equ 0 set "PYEXE=py -3.12"
echo [SELA Desktop] Interpreter Python: %PYEXE%

:: 4. Pastikan Mesin AI Offline aktif di port 8008
%PYEXE% -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8008/kesehatan', timeout=3)" >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo [SELA Desktop] Memulai server AI di latar belakang port 8008...
    start "SELA Mesin AI" /min cmd /c "%PYEXE% ai-engine/server.py"
...
:: 5. Meluncurkan Jendela Aplikasi Desktop SELA
